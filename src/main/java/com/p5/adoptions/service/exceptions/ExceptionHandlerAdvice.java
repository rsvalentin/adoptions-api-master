...
public class ExceptionHandlerAdvice extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex, HttpHeaders headers, HttpStatus status, WebRequest request) {
        return new ResponseEntity<>(new ApiError(HttpStatus.EXPECTATION_FAILED, "broken payload error", ex.getMessage()), status);
    }
    ...
    ApiError onConstraintValidationException(ConstraintViolationException ex) {
        ApiError error = new ApiError(HttpStatus.BAD_REQUEST, "Arguments validation failed error");
